...
class FileService:
    ...
    def validate_dataframe_structure(self, df: pd.DataFrame) -> Dict[str, Any]:
    ...
    async def process_excel_reviews(
        ...
        try:
            ...
            # Parse Excel file
            df = await self.parse_excel_file(file)
            ...
            # Validate structure
            column_mapping = self.validate_dataframe_structure(df)
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    # Create review
                    review_result = await review_service.create_and_process_review(
                    ...
                    if process_with_ai:
                        try:
                            ...
                            # Send email if negative sentiment and emails enabled
                            if (send_emails and 
                                analysis_result.get('should_send_email', False) and 
                                analysis_result.get('sentiment') == 'negative'):
                                
                                # Here you would call email service
                                # email_sent = await email_service.send_response_email(...)
            ...
            logger.info(f"✅ Excel processing complete: {results['processed']}/{results['total_rows']} rows processed")
